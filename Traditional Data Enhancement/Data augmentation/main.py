...
def TestOnePic():
    # test_jpg_loc = r"data/in-line resistor/1.jpg"
    test_jpg = cv2.imread(test_jpg_loc)
    cv2.imshow("Show Img", test_jpg)
    # cv2.waitKey(0)
    img1 = saturation(test_jpg)
...
def TestOneDir():
    ...
    for a, b, c in os.walk(root_path):
        for file_i in c:
            ...
            #
            # img_salt = SaltAndPepper(img_i,0.05)         ###不用
            # cv2.imwrite(os.path.join(save_path, file_i[:-4] + "_salt.jpg"), img_salt)
            #
            # img_salt = GaussianNoise(img_i,0.05)         ###不用
            # cv2.imwrite(os.path.join(save_path, file_i[:-4] + "_salt.jpg"), img_salt)
